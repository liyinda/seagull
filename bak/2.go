...
//列出deployment
func listDeployment(deploymentsClient v1beta1.DeploymentInterface)  {
    l, _ := deploymentsClient.List(metav1.ListOptions{})
    for {
        select {
        case e, _ := <-l.ResultChan():
            fmt.Println(e.Type, e.Object)
        }
    }
}
...
//创建deployment，需要谨慎按照部署的k8s版本来使用api接口
